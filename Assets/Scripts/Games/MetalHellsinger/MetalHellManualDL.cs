...
public class MetalHellManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData metalhellApworld;
    public FileDownloader.FileData metalhellAP;
    public FileDownloader.FileData metalhellBepInEx;

    [Header("GAME FOLDER NAMES")]
    public string steamGameFolderName = "Metal Hellsinger";
    ...
    private const string APModFolderName = "MetalHellsinger Archipelago";
    ...
    {
        public string metalhellAP;
        public string metalhellApworld;
        public string metalhellBepInEx;
    ...
    void ApplyGameConfig()
    {
        ...
        metalhellAP.url = remoteConfig.metalhellAP;
        metalhellApworld.url = remoteConfig.metalhellApworld;
        metalhellBepInEx.url = remoteConfig.metalhellBepInEx;
    }
    ...
    IEnumerator APWorldOnlyFlow()
    {
        ...
        {
            ShowInfo("Launching Metal: Hellsinger...");
            LaunchGame();
    ...
    IEnumerator InstallFlow(bool installAPWorld, bool installAPMod, bool installBepInEx)
    {
        ...
        if (installAPWorld)
        {
            ShowInfo("Installing Metal: Hellsinger APWorld...");
            yield return InstallAPWorld();
        }

        CreateVersionFile(metalhellAP.url, metalhellApworld.url, metalhellBepInEx.url);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "MetalHellBepInExTemp");

        yield return downloader.DownloadAndExtract(metalhellBepInEx, Application.persistentDataPath, extractPath);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "MetalHellAPModTemp");

        yield return downloader.DownloadAndExtract(metalhellAP, Application.persistentDataPath, extractPath);
        ...
        try
        {
            ...
            if (currentManifest != null)
                currentManifest.installedDirectories.Add(apModTargetPath);
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. APWorld URL: " + metalhellApworld.url);

        if (string.IsNullOrEmpty(metalhellApworld.url))
        {
        ...
        string fileName = metalhellApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = metalhellApworld.url.Substring(metalhellApworld.url.LastIndexOf('/') + 1);
        ...
        if (string.IsNullOrEmpty(fileName))
            fileName = "metal_hellsinger.apworld";

        string localPath = Path.Combine(Application.persistentDataPath, fileName);

        UnityEngine.Debug.Log("Downloading APWorld from: " + metalhellApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(metalhellApworld.url, localPath);
    ...
    void LaunchGame()
    {
        ...
        string exePath = Path.Combine(currentGamePath, "Metal.exe");
    ...
    {
        string manifestPath = Path.Combine(Application.persistentDataPath, "MetalHellInstalledFilesManifest.json");
    ...
    void CreateVersionFile(string apModUrl, string apworldUrl, string bepinexUrl)
    {
        try
        {
            ...
            string versionFileName = "Metal Hellsinger AP Version " + apModVersion + ".txt";
    ...
    void DeleteOldVersionFiles()
    {
        try
        {
            ...
            System.Text.RegularExpressions.Regex pattern = new System.Text.RegularExpressions.Regex(@"Metal Hellsinger AP Version .+\.txt");
