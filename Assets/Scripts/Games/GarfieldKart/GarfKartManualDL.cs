...
public class GarfKartManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData garfieldkartApworld;
    ...
    void ApplyGameConfig()
    {
        ...
        garfieldkartAP.url = remoteConfig.garfieldkartAP;
        garfieldkartApworld.url = remoteConfig.garfieldkartApworld;
        garfieldkartBepInEx.url = remoteConfig.garfieldkartBepInEx;
    }
    ...
    IEnumerator InstallFlow(bool installAPWorld, bool installAPMod, bool installBepInEx)
    {
        ...
        if (installAPWorld)
        {
            ShowInfo("Installing Garfield Kart APWorld...");
            yield return InstallAPWorld();
        }

        CreateVersionFile(garfieldkartAP.url, garfieldkartApworld.url, garfieldkartBepInEx.url);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "GarfKartBepInExTemp");

        yield return downloader.DownloadAndExtract(garfieldkartBepInEx, Application.persistentDataPath, extractPath);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "GarfKartAPModTemp");

        yield return downloader.DownloadAndExtract(garfieldkartAP, Application.persistentDataPath, extractPath);
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        string fileName = garfieldkartApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = garfieldkartApworld.url.Substring(garfieldkartApworld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + garfieldkartApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(garfieldkartApworld.url, localPath);
    ...
    void LaunchGame()
    {
        ...
        {
            Path.Combine(currentGamePath, "Garfield Kart Furious Racing.exe"),
            Path.Combine(currentGamePath, "GarfieldKartFuriousRacing.exe"),
        };
    ...
    void CloseGame()
    {
        try
        {
            Process[] processes = Process.GetProcessesByName("GarfieldKart");
    ...
    {
        string manifestPath = Path.Combine(Application.persistentDataPath, "GarfKartInstalledFilesManifest.json");
    ...
    void CreateVersionFile(string apModUrl, string apworldUrl, string bepinexUrl)
    {
        try
        {
            ...
            string versionFileName = "Garfield Kart AP Version " + apModVersion + ".txt";
