...
public class UnfairFlipsManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData unfairflipsApworld;
    ...
    void ApplyGameConfig()
    {
        ...
        unfairflipsAP.url = remoteConfig.unfairflipsAP;
        unfairflipsApworld.url = remoteConfig.unfairflipsApworld;
        unfairflipsBepInEx.url = remoteConfig.unfairflipsBepInEx;
    }
    ...
    IEnumerator InstallFlow(bool installAPWorld, bool installAPMod, bool installBepInEx)
    {
        ...
        if (installAPWorld)
        {
            ShowInfo("Installing Unfair Flips APWorld...");
            yield return InstallAPWorld();
        }

        CreateVersionFile(unfairflipsAP.url, unfairflipsApworld.url, unfairflipsBepInEx.url);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "UFBepInExTemp");

        yield return downloader.DownloadAndExtract(unfairflipsBepInEx, Application.persistentDataPath, extractPath);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "UFAPModTemp");

        yield return downloader.DownloadAndExtract(unfairflipsAP, Application.persistentDataPath, extractPath);
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. APWorld URL: " + unfairflipsApworld.url);

        if (string.IsNullOrEmpty(unfairflipsApworld.url))
        {
        ...
        string fileName = unfairflipsApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = unfairflipsApworld.url.Substring(unfairflipsApworld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + unfairflipsApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(unfairflipsApworld.url, localPath);
    ...
    void LaunchGame()
    {
        ...
        {
            Path.Combine(currentGamePath, "Unfair Flips.exe"),
        };
    ...
    void CloseGame()
    {
        try
        {
            Process[] processes = Process.GetProcessesByName("Unfair Flips");
    ...
    {
        string manifestPath = Path.Combine(Application.persistentDataPath, "UnfairFlipsInstalledFilesManifest.json");
    ...
    void CreateVersionFile(string apModUrl, string apworldUrl, string bepinexUrl)
    {
        try
        {
            ...
            string versionFileName = "Unfair Flips AP Version " + apModVersion + ".txt";
    ...
    void DeleteOldVersionFiles()
    {
        try
        {
            System.Text.RegularExpressions.Regex pattern = new System.Text.RegularExpressions.Regex(@"Unfair Flips AP Version .+\.txt");
