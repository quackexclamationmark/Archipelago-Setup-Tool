...
public class SUPERHOTManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("SUPERHOT FILES")]
    public FileDownloader.FileData superhotApworld;
    public FileDownloader.FileData superhotMelonLoader;
    public FileDownloader.FileData superhotAP;
    ...
    public string steamGameFolderName = "SUPERHOT";
    public string epicGameFolderName = "SUPERHOT";
    ...
    {
        public string superhotAP;
        public string superhotMelonLoader;
    ...
    void SelectSteam()
    {
        ...
        UnityEngine.Debug.Log("Switched to Steam - Path: " + superhotPath);
    }
    ...
    void SelectEpic()
    {
        ...
        superhotPath = GetSuperhotPath();
        UpdatePlatformStatus();
        UnityEngine.Debug.Log("Switched to Epic - Path: " + superhotPath);
    }
    ...
    void UpdatePlatformStatus()
    {
        if (platformStatus != null)
        {
            ...
            string status = string.IsNullOrEmpty(superhotPath) ? "Not Found" : "Found";
    ...
    private void ExecuteSetup()
    {
        ...
        if (needsGamePath && string.IsNullOrEmpty(superhotPath))
        {
    ...
    private void ExecuteRevert()
    {
        ...
        if (string.IsNullOrEmpty(superhotPath))
            return;

        string modsPath = Path.Combine(superhotPath, "Mods");
        ...
        if (removeAP)
        {
            ...
            SafeDeleteFile(Path.Combine(modsPath, "SuperhotArchipelago.dll"));
            SafeDeleteDirectory(Path.Combine(modsPath, "data"));
        ...
        if (fullClean)
        {
            ...
            SafeDeleteDirectory(Path.Combine(superhotPath, "MelonLoader"));
            SafeDeleteDirectory(Path.Combine(superhotPath, "UserData"));
            SafeDeleteDirectory(Path.Combine(superhotPath, "UserLibs"));
            SafeDeleteDirectory(Path.Combine(superhotPath, "Mods"));
            SafeDeleteDirectory(Path.Combine(superhotPath, "Plugins"));
            SafeDeleteFile(Path.Combine(superhotPath, "version.dll"));
    ...
    bool HasOtherMods(string modsPath)
    {
        ...
        foreach (string file in files)
        {
            ...
            if (name.StartsWith("SUPERHOT APMod Version") && name.EndsWith(".txt"))
                continue;

            if (name != "SuperhotArchipelago.dll")
                return true;
        }
        ...
        foreach (string dir in dirs)
        {
            ...
            if (string.Equals(name, "data", System.StringComparison.OrdinalIgnoreCase))
                continue;
    ...
    IEnumerator InstallFlow()
    {
        ...
        CreateVersionFile(superhotAP.url, superhotMelonLoader.url, superhotApworld.url);
    ...
    IEnumerator InstallAPMod()
    {
        ...
        if (string.IsNullOrEmpty(superhotAP.url))
        {
        ...
        string extractPath = Path.Combine(Application.persistentDataPath, "SUPERHOT_AP_Extract");

        yield return downloader.DownloadAndExtract(superhotAP, Application.persistentDataPath, extractPath);

        UnityEngine.Debug.Log("AP Mod extraction complete");

        CopyExtractedFiles(extractPath, superhotPath);

        Directory.CreateDirectory(Path.Combine(superhotPath, "Mods"));
    ...
    IEnumerator InstallMelonLoader()
    {
        ...
        if (string.IsNullOrEmpty(superhotMelonLoader.url))
        {
        ...
        string extractPath = Path.Combine(Application.persistentDataPath, "SUPERHOT_MelonTemp");

        yield return downloader.DownloadAndExtract(superhotMelonLoader, Application.persistentDataPath, extractPath);
    ...
    void CopyExtractedFiles(string extractPath, string targetPath)
    {
        ...
        try
        {
            ...
            UnityEngine.Debug.Log("All extracted files copied to game directory");
        }
        catch (System.Exception e)
        {
    ...
    void LaunchSuperhot()
    {
        try
        {
            string shPath = Path.Combine(superhotPath, "SH.exe");
            string superhotExePath = Path.Combine(superhotPath, "SUPERHOT.exe");

            string exePath = File.Exists(shPath) ? shPath : superhotExePath;

            if (File.Exists(exePath))
            {
                Process.Start(exePath);
                UnityEngine.Debug.Log("SUPERHOT launched: " + exePath);
            }
            else
            {
                UnityEngine.Debug.LogError("Neither SH.exe nor SUPERHOT.exe found in: " + superhotPath);
                ShowInfo("ERROR: SH.exe / SUPERHOT.exe not found!");
            }
        }
        catch (System.Exception e)
        {
            UnityEngine.Debug.LogError("Failed to launch SUPERHOT: " + e.Message);
            ShowInfo("ERROR: Failed to launch SUPERHOT!");
        }
    }
    ...
    void CreateVersionFile(string apUrl, string melonloaderUrl, string apworldUrl)
    {
        try
        {
            ...
            string versionFileName = "SUPERHOT APMod Version " + apVersion + ".txt";
            string content = "SUPERHOT Archipelago Setup Tool\n";
            ...
            string rootVersionPath = Path.Combine(superhotPath, versionFileName);
    ...
    void DeleteOldVersionFiles()
    {
        try
        {
            System.Text.RegularExpressions.Regex pattern = new System.Text.RegularExpressions.Regex(@"SUPERHOT APMod Version .+\.txt");

            if (!string.IsNullOrEmpty(superhotPath))
            {
                string[] rootFiles = Directory.GetFiles(superhotPath);
    ...
    string GetSuperhotPath()
    {
        if (isEpic)
            return GetSuperhotEpicPath();
        else
            return GetSuperhotSteamPath();
    }

    string GetSuperhotSteamPath()
    {
    ...
    IEnumerator LoadRemoteConfig()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(url))
        {
            ...
            try
            {
                remoteConfig = JsonUtility.FromJson<SuperhotConfig>(request.downloadHandler.text);
    ...
    void ApplySuperhotConfig()
    {
        if (remoteConfig == null)
            return;

        if (superhotAP != null && !string.IsNullOrEmpty(remoteConfig.superhotAP))
            superhotAP.url = remoteConfig.superhotAP;

        if (superhotMelonLoader != null && !string.IsNullOrEmpty(remoteConfig.superhotMelonLoader))
            superhotMelonLoader.url = remoteConfig.superhotMelonLoader;
