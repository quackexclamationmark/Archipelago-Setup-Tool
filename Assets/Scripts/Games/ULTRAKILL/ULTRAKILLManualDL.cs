...
public class ULTRAKILLManualDL : MonoBehaviour
{
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. APWorld URL: " + ultrakillAPWorld.url);

        if (string.IsNullOrEmpty(ultrakillAPWorld.url))
        {
        ...
        string fileName = ultrakillAPWorld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = ultrakillAPWorld.url.Substring(ultrakillAPWorld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + ultrakillAPWorld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(ultrakillAPWorld.url, localPath);
