...
public class BluePrinceManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData blueprinceApworld;
    public FileDownloader.FileData blueprinceAP;
    public FileDownloader.FileData blueprinceBepInEx;

    [Header("GAME FOLDER NAMES")]
    public string steamGameFolderName = "Blue Prince";
    ...
    private const string ApModFolderName = "Blue Prince Archipelago";
    private const string ApModFolderNameLegacy = "BluePrinceArchipelago";
    ...
    {
        public string blueprinceAP;
        public string blueprinceApworld;
        public string blueprinceBepInEx;
    ...
    void ApplyGameConfig()
    {
        ...
        blueprinceAP.url = remoteConfig.blueprinceAP;
        blueprinceApworld.url = remoteConfig.blueprinceApworld;
        blueprinceBepInEx.url = remoteConfig.blueprinceBepInEx;
    }
    ...
    private void ExecuteSetup()
    {
        ...
        StartCoroutine(SetupFlow(apworld, apmod, bepinex));
    }
    ...
    IEnumerator SetupFlow(bool installAPWorld, bool installAPMod, bool installBepInEx)
    {
    ...
    private void ExecuteRevert()
    {
        ...
        if (clearAP)
        {
            ...
            string apModPath = Path.Combine(pluginsPath, ApModFolderName);
            SafeDeleteDirectory(apModPath);

            string apModPathLegacy = Path.Combine(pluginsPath, ApModFolderNameLegacy);
            SafeDeleteDirectory(apModPathLegacy);
    ...
    bool HasOtherMods(string pluginsPath)
    {
        ...
        foreach (string dir in dirs)
        {
            ...
            if (dirName == ApModFolderName || dirName == ApModFolderNameLegacy)
                continue;
    ...
    IEnumerator InstallFlow(bool installAPWorld, bool installAPMod, bool installBepInEx)
    {
        ...
        if (installAPWorld)
        {
            ShowInfo("Installing Blue Prince APWorld...");
            yield return InstallAPWorld();
        }

        CreateVersionFile(blueprinceAP.url, blueprinceApworld.url, blueprinceBepInEx.url);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "BluePrinceBepInExTemp");

        yield return downloader.DownloadAndExtract(blueprinceBepInEx, Application.persistentDataPath, extractPath);
    ...
    {
        string extractPath = Path.Combine(Application.persistentDataPath, "BluePrinceAPModTemp");

        yield return downloader.DownloadAndExtract(blueprinceAP, Application.persistentDataPath, extractPath);
        ...
        string pluginsPath = Path.Combine(gamePath, "BepInEx", "plugins");
        string apModTargetPath = Path.Combine(pluginsPath, ApModFolderName);

        try
        {
            Directory.CreateDirectory(pluginsPath);

            Directory.CreateDirectory(apModTargetPath);

            foreach (string file in Directory.GetFiles(extractPath, "*", SearchOption.AllDirectories))
            {
                string relativePath = file.Substring(extractPath.Length).TrimStart(Path.DirectorySeparatorChar);
                string dest = Path.Combine(apModTargetPath, relativePath);
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. APWorld URL: " + blueprinceApworld.url);

        if (string.IsNullOrEmpty(blueprinceApworld.url))
        {
        ...
        string fileName = blueprinceApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = blueprinceApworld.url.Substring(blueprinceApworld.url.LastIndexOf('/') + 1);
        ...
        if (string.IsNullOrEmpty(fileName))
            fileName = "blueprince.apworld";

        string localPath = Path.Combine(Application.persistentDataPath, fileName);

        UnityEngine.Debug.Log("Downloading APWorld from: " + blueprinceApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(blueprinceApworld.url, localPath);
    ...
    void LaunchGame()
    {
        ...
        {
            Path.Combine(currentGamePath, "BLUE PRINCE.exe"),
        };
    ...
    void CloseGame()
    {
        try
        {
            Process[] processes = Process.GetProcessesByName("BLUE PRINCE");
    ...
    void CreateSteamAppIdFile()
    {
        try
        {
            ...
            {
                UnityEngine.Debug.LogWarning("CreateSteamAppIdFile: gamePath is empty or doesn't exist. Skipping steam_appid.txt creation.");
                return;
            }

            string appIdPath = Path.Combine(gamePath, "steam_appid.txt");
            File.WriteAllText(appIdPath, "1569580");
            UnityEngine.Debug.Log("steam_appid.txt created: " + appIdPath);
        }
        catch (System.Exception e)
        {
    ...
    void CreateVersionFile(string apModUrl, string apworldUrl, string bepinexUrl)
    {
        try
        {
            ...
            string versionFileName = "Blue Prince AP Version " + apModVersion + ".txt";
    ...
    void DeleteOldVersionFiles()
    {
        try
        {
            ...
            System.Text.RegularExpressions.Regex pattern = new System.Text.RegularExpressions.Regex(@"Blue Prince AP Version .+\.txt");
