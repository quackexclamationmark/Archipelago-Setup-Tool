...
public class PokePlatManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData pokemonplatinumBizHawk;
    public FileDownloader.FileData pokemonplatinumApworld;
    ...
    IEnumerator RunInstallBiz()
    {
        ...
        if (pokemonplatinumBizHawk == null)
        {
            ShowInfo("ERROR: pokemonplatinumBizHawk not configured!");
            yield break;
        }

        if (string.IsNullOrEmpty(pokemonplatinumBizHawk.url))
        {
            ShowInfo("ERROR: pokemonplatinumBizHawk URL is empty!");
            yield break;
        }

        string url = pokemonplatinumBizHawk.url;
        string fileName = pokemonplatinumBizHawk.fileName;
        ...
        string extractPath = Path.Combine(Application.persistentDataPath, "BizHawkTemp_Platinum");
        ...
        if (extractionFailed)
        {
            ...
            if (downloader != null)
            {
                yield return downloader.DownloadAndExtract(pokemonplatinumBizHawk, Application.persistentDataPath, extractPath);
            }
            else
            {
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + pokemonplatinumApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(pokemonplatinumApworld.url, localPath);
    ...
    void ApplyRemoteConfig()
    {
        ...
        pokemonplatinumBizHawk.url = remoteConfig.pokemonplatinumBizHawk;
        pokemonplatinumApworld.url = remoteConfig.pokemonplatinumApworld;
        UnityEngine.Debug.Log("Applied pokemonplatinumBizHawk URL: " + pokemonplatinumBizHawk.url);
        UnityEngine.Debug.Log("Applied pokemonplatinumApworld URL: " + pokemonplatinumApworld.url);
    }
