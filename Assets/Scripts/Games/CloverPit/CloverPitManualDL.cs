...
public class CloverPitManualDL : MonoBehaviour
{
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. APWorld URL: " + cloverPitAPWorld.url);

        if (string.IsNullOrEmpty(cloverPitAPWorld.url))
        {
        ...
        string fileName = cloverPitAPWorld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = cloverPitAPWorld.url.Substring(cloverPitAPWorld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + cloverPitAPWorld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(cloverPitAPWorld.url, localPath);
