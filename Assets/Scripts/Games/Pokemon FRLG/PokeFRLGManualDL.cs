...
public class PokeFRLGManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData pokemonfrlgBizHawk;
    public FileDownloader.FileData pokemonfrlgApworld;
    ...
    {
        public string pokemonfrlgBizHawk;
        public string pokemonfrlgApworld;
    }
    ...
    IEnumerator RunInstallBiz()
    {
        ...
        if (pokemonfrlgBizHawk == null)
        {
            ShowInfo("ERROR: pokemonfrlgBizHawk not configured!");
            yield break;
        }

        if (string.IsNullOrEmpty(pokemonfrlgBizHawk.url))
        {
            ShowInfo("ERROR: pokemonfrlgBizHawk URL is empty!");
            yield break;
        }

        string url = pokemonfrlgBizHawk.url;
        string fileName = pokemonfrlgBizHawk.fileName;
        ...
        string extractPath = Path.Combine(Application.persistentDataPath, "BizHawkTemp_FRLG");
        ...
        if (extractionFailed)
        {
            ...
            if (downloader != null)
            {
                yield return downloader.DownloadAndExtract(pokemonfrlgBizHawk, Application.persistentDataPath, extractPath);
            }
            else
            {
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. FRLG APWorld URL: " + pokemonfrlgApworld.url);

        if (string.IsNullOrEmpty(pokemonfrlgApworld.url))
        {
        ...
        string fileName = pokemonfrlgApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = pokemonfrlgApworld.url.Substring(pokemonfrlgApworld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + pokemonfrlgApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(pokemonfrlgApworld.url, localPath);
    ...
    void ApplyRemoteConfig()
    {
        ...
        pokemonfrlgBizHawk.url = remoteConfig.pokemonfrlgBizHawk;
        pokemonfrlgApworld.url = remoteConfig.pokemonfrlgApworld;
        UnityEngine.Debug.Log("Applied pokemonfrlgBizHawk URL: " + pokemonfrlgBizHawk.url);
        UnityEngine.Debug.Log("Applied pokemonfrlgApworld URL: " + pokemonfrlgApworld.url);
    }
