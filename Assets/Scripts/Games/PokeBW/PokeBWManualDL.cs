...
public class PokeBWManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData pokemonbwBizHawk;
    public FileDownloader.FileData pokemonbwApworld;
    ...
    {
        public string pokemonbwBizHawk;
        public string pokemonbwApworld;
    }
    ...
    IEnumerator RunInstallBiz()
    {
        ...
        if (pokemonbwBizHawk == null)
        {
            ShowInfo("ERROR: pokemonbwBizHawk not configured!");
            yield break;
        }

        if (string.IsNullOrEmpty(pokemonbwBizHawk.url))
        {
            ShowInfo("ERROR: pokemonbwBizHawk URL is empty!");
            yield break;
        }

        string url = pokemonbwBizHawk.url;
        string fileName = pokemonbwBizHawk.fileName;
        ...
        string extractPath = Path.Combine(Application.persistentDataPath, "BizHawkTemp_BW");
        ...
        if (extractionFailed)
        {
            ...
            if (downloader != null)
            {
                yield return downloader.DownloadAndExtract(pokemonbwBizHawk, Application.persistentDataPath, extractPath);
            }
            else
            {
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. BW APWorld URL: " + pokemonbwApworld.url);

        if (string.IsNullOrEmpty(pokemonbwApworld.url))
        {
        ...
        string fileName = pokemonbwApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            // For Pokemon Black/White, default filename is pokemon_bw.apworld
            fileName = "pokemon_bw.apworld";
            UnityEngine.Debug.Log("Using default APWorld filename: " + fileName);
        }

        string localPath = Path.Combine(Application.persistentDataPath, fileName);

        UnityEngine.Debug.Log("Downloading APWorld from: " + pokemonbwApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(pokemonbwApworld.url, localPath);
    ...
    void ApplyRemoteConfig()
    {
        ...
        pokemonbwBizHawk.url = remoteConfig.pokemonbwBizHawk;
        pokemonbwApworld.url = remoteConfig.pokemonbwApworld;
        UnityEngine.Debug.Log("Applied pokemonbwBizHawk URL: " + pokemonbwBizHawk.url);
        UnityEngine.Debug.Log("Applied pokemonbwApworld URL: " + pokemonbwApworld.url);
    }
