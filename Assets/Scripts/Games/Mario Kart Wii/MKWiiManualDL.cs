...
public class MKWiiManualDL : MonoBehaviour
{
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. APWorld URL: " + mkwiiApworld.url);

        if (string.IsNullOrEmpty(mkwiiApworld.url))
        {
        ...
        string fileName = mkwiiApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = mkwiiApworld.url.Substring(mkwiiApworld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + mkwiiApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(mkwiiApworld.url, localPath);
