...
public class TLOZTMCManualDL : MonoBehaviour
{
    public FileDownloader downloader;

    [Header("GAME FILES")]
    public FileDownloader.FileData tloztmcBizHawk;
    public FileDownloader.FileData tloztmcApworld;
    ...
    {
        public string tloztmcBizHawk;
        public string tloztmcApworld;
    }
    ...
    IEnumerator RunInstallBiz()
    {
        ...
        if (tloztmcBizHawk == null)
        {
            ShowInfo("ERROR: tloztmcBizHawk not configured!");
            yield break;
        }

        if (string.IsNullOrEmpty(tloztmcBizHawk.url))
        {
            ShowInfo("ERROR: tloztmcBizHawk URL is empty!");
            yield break;
        }

        string url = tloztmcBizHawk.url;
        string fileName = tloztmcBizHawk.fileName;
        ...
        string extractPath = Path.Combine(Application.persistentDataPath, "BizHawkTemp_TMC");
        ...
        if (extractionFailed)
        {
            ...
            if (downloader != null)
            {
                yield return downloader.DownloadAndExtract(tloztmcBizHawk, Application.persistentDataPath, extractPath);
            }
            else
            {
    ...
    IEnumerator InstallAPWorld()
    {
        ...
        UnityEngine.Debug.Log("Config loaded. TMC APWorld URL: " + tloztmcApworld.url);

        if (string.IsNullOrEmpty(tloztmcApworld.url))
        {
        ...
        string fileName = tloztmcApworld.fileName;
        if (string.IsNullOrEmpty(fileName))
        {
            fileName = tloztmcApworld.url.Substring(tloztmcApworld.url.LastIndexOf('/') + 1);
        ...
        UnityEngine.Debug.Log("Downloading APWorld from: " + tloztmcApworld.url);
        UnityEngine.Debug.Log("Saving to: " + localPath);

        yield return DownloadFile(tloztmcApworld.url, localPath);
    ...
    void ApplyRemoteConfig()
    {
        ...
        tloztmcBizHawk.url = remoteConfig.tloztmcBizHawk;
        tloztmcApworld.url = remoteConfig.tloztmcApworld;
        UnityEngine.Debug.Log("Applied tloztmcBizHawk URL: " + tloztmcBizHawk.url);
        UnityEngine.Debug.Log("Applied tloztmcApworld URL: " + tloztmcApworld.url);
    }
