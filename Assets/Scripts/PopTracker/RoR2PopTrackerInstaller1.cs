...
public class RoR2PopTrackerInstaller : MonoBehaviour
{
    [Header("UI ELEMENTS")]
    ...
    private string ror2poptrackerDownloadUrl = "";
    ...
    {
        Debug.Log("Loading RoR2 PopTracker config from: " + configUrl);
        ...
        try
        {
            ...
            ror2poptrackerDownloadUrl = remoteConfig.ror2PopTracker;
            Debug.Log("RoR2 PopTracker config loaded successfully.");
            Debug.Log("PopTracker URL: " + poptrackerDownloadUrl);
            Debug.Log("RoR2 PopTracker Pack URL: " + ror2poptrackerDownloadUrl);
        }
    ...
    void InstallPopTracker()
    {
        ...
        if (string.IsNullOrEmpty(ror2poptrackerDownloadUrl))
        {
            ShowInfo("RoR2 PopTracker download URL not loaded");
            return;
        }
    ...
    IEnumerator DownloadAndInstall()
    {
        ...
        string tempDownloadPath = Path.Combine(Application.persistentDataPath, "RoR2PopTrackerTemp");
        ...
        // STEP 4: Download and install RoR2 PopTracker pack
        if (string.IsNullOrEmpty(ror2poptrackerDownloadUrl))
        {
            ShowInfo("ERROR: RoR2 PopTracker URL not loaded");
            installButton.interactable = true;
            SafeDeleteDirectory(tempDownloadPath);
            yield break;
        }

        ShowInfo("Downloading RoR2 PopTracker pack...");
        string ror2PackZipPath = Path.Combine(tempDownloadPath, "RoR2PopTracker.zip");

        yield return DownloadFile(ror2poptrackerDownloadUrl, ror2PackZipPath);

        if (!File.Exists(ror2PackZipPath))
        {
            ShowInfo("ERROR: RoR2 PopTracker pack download failed!");
            installButton.interactable = true;
            SafeDeleteDirectory(tempDownloadPath);
            yield break;
        }

        ShowInfo("Installing RoR2 PopTracker pack...");
        ...
        string packDestZipPath = Path.Combine(packsPath, "RoR2APPoptracker.zip");
        File.Copy(ror2PackZipPath, packDestZipPath, true);
        Debug.Log("RoR2 PopTracker pack copied to: " + packDestZipPath);
